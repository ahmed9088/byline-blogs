...
import { 
  Clock, ArrowLeft, Layers, BookOpen, Cpu, Layout, Brain, 
  Shield, LineChart, Activity, Trophy, Film, Compass, Globe, 
  Book, Atom, MapPin, Utensils, Palette, Sparkles 
} from 'lucide-react';
    ...
    const fetchCategoryPosts = async () => {
      ...
      try {
        ...
        if (catRes.data.success) {
          ...
          setCurrentCategory(matched || { name: slug, description: '' });
        }

        const postsRes = await postsAPI.getPosts({ category: slug as string, status: 'published', limit: 30 });
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center space-y-4">
        <SEOHead title="Loading Section - Byline" />
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      <SEOHead 
        ...
        description={currentCategory?.description || 'Browse curated editorial articles.'}
      ...
      <BreadcrumbJSONLD
        items={[
          ...
          { name: "All Categories", url: "http://localhost:3000/category" },
          { name: currentCategory?.name || "Category", url: `http://localhost:3000/category/${slug}` },
        ]}
      />
