...
import { ArrowLeft, KeyRound, Mail, User, Eye, EyeOff } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    const fetchSiteName = async () => {
      try {
        ...
        console.error('Register settings fetch error:', err.message);
      }
    };
    fetchSiteName();
  }, [user, router]);
  ...
  return (
    <div className="min-h-[80vh] flex flex-col justify-center px-4">
      <SEOHead title="Register Account - Byline" />
        ...
        <div className="bg-white dark:bg-zinc-950 p-8 border border-neutral-200/50 dark:border-neutral-850 rounded-2xl shadow-xs space-y-6">
          <div className="flex flex-col items-center justify-center space-y-3 text-center">
            ...
            <h2 className="font-sans text-lg font-bold tracking-wider uppercase text-neutral-900 dark:text-neutral-50">
              Create a Bylines.dev Account
            </h2>
            <p className="text-[10px] text-neutral-400 uppercase tracking-widest font-semibold">
              Join the technical publishing community
            </p>
          </div>

          {error && (
            <div className="text-[11px] bg-red-50 dark:bg-red-955/20 text-red-500 border border-red-100/50 dark:border-red-900 p-2.5 rounded-lg">
              {error}
            </div>
          )}
          ...
          <div className="text-center text-[10px] text-neutral-450 dark:text-neutral-500 border-t border-neutral-100 dark:border-neutral-900 pt-4">
            ...
            <Link href="/login" className="text-editorial-accent font-semibold hover:underline">
              Sign in
            </Link>
          </div>
        </div>
