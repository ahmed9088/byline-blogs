...
export default function TagArchive() {
  ...
  useEffect(() => {
    const fetchTagPosts = async () => {
      ...
      try {
        ...
        const postsRes = await postsAPI.getPosts({ tag: slug as string, status: 'published', limit: 20 });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      <SEOHead 
        title={tagName ? `#${tagName} Articles - Byline` : 'Tag Archive'} 
        description={`Browse articles tagged with #${tagName || slug}.`}
      />
